const BASE_URL = window.location.hostname === "localhost"
    ...
    : "https://doclink-production.up.railway.app";  // Correct production URL on Railway

async function fetchDoctors() {
    try {
        const response = await fetch(`${BASE_URL}/api/doctors`); // Use BASE_URL
...
async function loadAvailability(card, doctorId, doctorName) {
    try {
        const res = await fetch(`${BASE_URL}/api/availability/${doctorId}`); // Use BASE_URL
...
async function loadReviews(doctorId, card) {
    try {
        const res = await fetch(`${BASE_URL}/api/reviews/doctor/${doctorId}`); // Use BASE_URL
